...
public class DataHandlerService {
    ...
    public DataHandlerService(MdmsV2Util mdmsV2Util, HrmsUtil hrmsUtil, LocalizationUtil localizationUtil, TenantManagementUtil tenantManagementUtil, ServiceConfiguration serviceConfig, ObjectMapper objectMapper, ResourceLoader resourceLoader, WorkflowUtil workflowUtil, CustomKafkaTemplate producer, BillingSlabUtil billingSlabUtil) {
        ...
        this.billingSlabUtil = billingSlabUtil;
    }

    // this is default data create is basically done by mdms-v2 service
    public void createDefaultData(DefaultDataRequest defaultDataRequest) {
        if (defaultDataRequest.getSchemaCodes() != null) {
            ...
            DefaultMdmsDataRequest defaultMdmsDataRequest = DefaultMdmsDataRequest.builder().
                    ...
                    defaultTenantId(serviceConfig.getDefaultTenantId()).build();
    ...
    public DefaultDataRequest setupDefaultData(DataSetupRequest dataSetupRequest) {
        DefaultDataRequest defaultDataRequest = DefaultDataRequest.builder().
                ...
                onlySchemas(dataSetupRequest.getOnlySchemas()).build();

        //pushes workflow config with the given tenantId in the request
        ...
        if (dataSetupRequest.getSchemaCodes() == null) {
            if (Objects.equals(dataSetupRequest.getModule(), "PGR") && serviceConfig.getEnabledModules().contains(dataSetupRequest.getModule())) {
                defaultDataRequest.setSchemaCodes(serviceConfig.getMdmsSchemacodeMap().get("PGR"));
            }
            if (Objects.equals(dataSetupRequest.getModule(), "HRMS") && serviceConfig.getEnabledModules().contains(dataSetupRequest.getModule())) {
                defaultDataRequest.setSchemaCodes(serviceConfig.getMdmsSchemacodeMap().get("HRMS"));
            }
            if (Objects.equals(dataSetupRequest.getModule(), "TL") && serviceConfig.getEnabledModules().contains(dataSetupRequest.getModule())) {
                defaultDataRequest.setSchemaCodes(serviceConfig.getMdmsSchemacodeMap().get("TL"));
            }
        } else {
...
//    creates workflow for the given module
    public void createWorkflowConfig(String targetTenantId, String module){
        String classPath = "classPath:"+module+"WorkflowConfig.json";
    ...
    public void createBillingSlabs(String targetTenantId, String module){
        String classPath = "classPath:"+module+"BillingSlabs.json";
        Resource resource = resourceLoader.getResource(classPath);
        try(InputStream inputStream = resource.getInputStream()){
            BillingSlabReq billingSlabReq = objectMapper.readValue(inputStream, BillingSlabReq.class);
            billingSlabReq.getBillingSlab().forEach(slab -> slab.setTenantId(targetTenantId));
            billingSlabUtil.createBillingSlabConfig(billingSlabReq);
        }catch (Exception e){
            throw new CustomException("IO_EXCEPTION", "Error reading or mapping JSON file: " + e.getMessage());
        }
    }

    // adding only roleactions master as of now
