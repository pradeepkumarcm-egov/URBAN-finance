...
const useTLDocumentSearch = (data1 = {}, config = {}) => {
  ...
  const { isLoading, error, data } = useQuery(
    [`tlDocuments-${1}`, filesArray],
    async () => {
      let tenantInfo = window?.globalConfigs?.getConfig("ENABLE_SINGLEINSTANCE") ? `?tenantId=${tenant}` : "";

      let params = window?.globalConfigs?.getConfig("ENABLE_SINGLEINSTANCE")
        ? { fileStoreIds: filesArray?.join(",") }
        : { tenantId: tenant, fileStoreIds: filesArray?.join(",") };
      const FileFetchUrl = `/filestore/v1/files/url`;
      const response = await Digit.CustomService.getResponse({
        url: `${FileFetchUrl}${tenantInfo}`,
        method: "GET",
        params: params,
        auth: false,
        useCache: false,
