...
const useTLSearch = (params, config) => {
  return async () => {
    ...
    const workflowRes = await Digit.CustomService.getResponse({
      url: `/egov-workflow-v2/egov-wf/process/_search`,
      method: "POST",
      params: { tenantId: tenant, businessIds: businessIds.join() },
      auth: true,
      useCache: false,
    });
  ...
  const result = useQuery(["TL_APPLICATIONS_LIST", params], useTLSearch(params, config), {
    ...
    select: (data) => {
      return data?.map((i) => ({
        ...
        TL_COMMON_TABLE_COL_SLA_NAME: i?.status.match(/^(EXPIRED|APPROVED|CANCELLED)$/)
          ? "CS_NA"
          : `${Math.round(i?.SLA / (1000 * 60 * 60 * 24))} ${t("TL_SLA_DAYS")}`,
