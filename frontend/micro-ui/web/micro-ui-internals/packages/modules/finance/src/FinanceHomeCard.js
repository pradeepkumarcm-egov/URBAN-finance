...
import LandingPageCard from "./components/molecules/LandingPageCard";

const FinanceCard = () => {
  const { t } = useTranslation();
  const userRoles = Digit.SessionStorage.get("User")?.info?.roles;
  const isEmployee = userRoles.find((role) => role.code === "EMPLOYEE_FINANCE");
  ...
  return (
    <div>
      ...
      <LandingPageCard {...propsForModuleCard} />
    </div>
  )
};
