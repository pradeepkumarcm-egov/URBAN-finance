...
searchAndDownloadConfig: {
  ...
  additionalCustomizations: (row, key, column, value, t, searchResult) => {
    // These console logs are good for debugging, kept them in.
    // console.log("Row:", row);
    // console.log("Key:", key);
    // console.log("Column:", column);

    // Get tenantId from the first result for button components
    const tenantId = searchResult?.[0]?.tenantid;
    const counter = row?.counter;

    switch (key) {
      // Key should match the 'key' defined in your searchResult columns
      case "action": // <-- CHANGED Key from "Action"
        if (counter === 0) {
          // Assuming DownloadButton and PayAndDownloadButton are reusable components
      ...
      // Key should match the 'key' defined in your searchResult columns
      case "dateOfBirth": // <-- CHANGED Key from "Death Date"
        const epoch = row?.dateofbirth; // <-- CHANGED property from row.dateofdeath
        if (epoch) {
          const date = new Date(epoch);
      ...
      default:
        // This is a fallback, but usually not hit if columns are well-defined
        return t("ES_COMMON_NA");
    }
  },
},
